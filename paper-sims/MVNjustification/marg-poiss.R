...
test <- function(b, Y, X, Z, beta, D, S, SS, Fu, Fi, l0i, haz, Del, gam, zet,
                 survival = TRUE){
  eta <- X %*% beta + Z %*% b
  out <- prod(dpois(Y, exp(eta))) * mvtnorm::dmvnorm(b, sigma = D) 
  if(survival) out <- out * exp(logfti(b, S, SS, Fi, Fu, l0i, haz, Del, gam, zet))
  out
}

# 70% failure rates
A <- .sim(100, 5,  'poisson', fup = 15, unif.times = F)
B <- .sim(100, 10, 'poisson', fup = 15, unif.times = F)
C <- .sim(100, 15, 'poisson', fup = 15, unif.times = F)
fun1R <- function(dat, survival){
  ...
  out <- Map(function(X, Y, Z, S, SS, Fu, Fi, l0i, haz, Del){
    a <- tryCatch(MCMCmetrop1R(test, theta.init = c(0,0), 
                 Y = Y, X = X, Z = Z, beta = c(2, -.1, 0.1, .2), D = diag(c(.25, .05)),
